...
三种入口协议（CC / AM / RP）与上游能力的组合决定透传或转换。**透传（Passthrough）要求上游返回的字节格式等于入口协议期望的格式**——这是字节透传的硬约束，不能靠 allow 标志绕过。

fustapi 的 provider 均为 OpenAI-compatible（CC-up），无原生 AM-up；Responses 上游通过 `supports_responses` 标识（RP-up）。由此矩阵：

| 入口协议 | 上游 CC-up（DeepSeek 等）| 上游 RP-up（supports_responses）|
|---------|------------------------|-------------------------------|
| CC（/v1/chat/completions）| **透传**（格式匹配）| — |
| AM（/v1/messages）| **转换**（格式不匹配，`AnthropicStreamState`）| — |
| RP（/v1/responses）| **转换**（格式不匹配，`ResponsesStreamState`）| **透传**（格式匹配）|

带 `—` 的组合不在范围（fustapi 无 AM-up；CC→RP-up 反向未支持）。

### 候选 #4 回归修正（前置）

架构重构候选 #4 将 `allow_passthrough` 改为无条件 `true`，假设"无 transform 即可透传"。但这忽略了协议格式匹配：AM 入口透传到 CC-up 会产出混合格式垃圾（`message_start` + OpenAI `chat.completion.chunk` + `message_stop`）。

验证数据流（当前代码，AM 入口 + CC-up + 流式）：

```
forward_streaming: allow_passthrough = true
→ router.chat_stream(req, true): 无 transform → 保持 true
→ openai.rs:427 if allow_passthrough → Passthrough(上游 CC SSE 字节)
→ stream_dispatch:149 if protocol==Anthropic → wrap_anthropic(CC 字节)
→ 客户端收到混合格式 ← 错误
```

修正：透传判定恢复 protocol-aware，语义改为"入口协议 == 上游协议"。fustapi 现实（无 AM-up）下等价于：
- CC 入口 → CC-up：`allow_passthrough`（无 transform 时为 true）
- AM 入口 → CC-up：强制转换（`allow_passthrough = false`）
- RP 入口 → RP-up（supports_responses）：透传
- RP 入口 → CC-up：转换

本设计将此修正作为实现前置（实现顺序第 0 步），并为 AM→CC-up 透传回归添加测试。
...
| `protocol/mod.rs` | `Protocol` 加 `Responses`；`detect_protocol` 加 `/v1/responses` 分支；`dispatch_request` 加 responses handler 路由；`forward_streaming` 恢复 protocol-aware `allow_passthrough`（修正候选 #4） |
...
**透传判定回归**
- AM 入口 + CC-up → 强制转换（验证不走 Passthrough，不产混合格式）
- CC 入口 + CC-up → 透传（格式匹配）
- RP 入口 + RP-up → 透传；RP 入口 + CC-up → 转换
...
0. **修正透传判定**：恢复 protocol-aware `allow_passthrough`（修复候选 #4 的 AM→CC-up 混合格式 bug）+ 回归测试
