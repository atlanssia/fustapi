...
2. Open `http://localhost:8800/ui` in your browser.
...
```bash
# Default settings (127.0.0.1:8800)
fustapi serve
...
- `--port PORT`: Port number (default: `8800`)
...
Type=simple
User=fustapi
ExecStart=/usr/local/bin/fustapi serve --host 0.0.0.0 --port 8800
Restart=on-failure
RestartSec=5
