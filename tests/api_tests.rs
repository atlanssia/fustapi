...
async fn update_provider_empty_name() {
    ...
    let req = json_request("PUT", "/api/providers/up-empty-name", json!({
        "name": "",
        "type": "omlx",
        "endpoint": "http://localhost:8000/v1"
    }));
...
async fn update_provider_invalid_type() {
    ...
    let req = json_request("PUT", "/api/providers/up-bad-type", json!({
        "name": "up-bad-type",
        "type": "bad_type",
        "endpoint": "http://localhost:8000/v1"
    }));
...
async fn create_route_empty_model_with_existing_provider() {
    ...
    assert!(status == StatusCode::OK || status == StatusCode::BAD_REQUEST, "body: {body}");
}
...
async fn update_existing_provider() {
    ...
    let req = json_request("PUT", "/api/providers/update-target", json!({
        "name": "update-target",
        "type": "openai",
        "endpoint": "https://api.openai.com/v1",
        "api_key": "sk-new-key"
...
async fn update_provider_rename_affects_routes() {
    ...
    // Rename provider
    let req = json_request("PUT", "/api/providers/rename-p", json!({
        "name": "renamed-p",
        "type": "omlx",
        "endpoint": "http://localhost:8000/v1"
    }));
...
async fn delete_provider_does_not_affect_other_routes() {
    ...
    assert_eq!(s, StatusCode::OK);
    let v: Value = serde_json::from_str(&body).unwrap();
    let models: Vec<&str> = v["models"].as_array().unwrap().iter()
        .filter_map(|m| m["id"].as_str())
        .collect();
    assert!(models.contains(&"keep-model"), "keep-model should still exist after deleting unrelated provider");
}
...
async fn sql_injection_preserves_database_integrity() {
    ...
    assert_eq!(s, StatusCode::OK);
    let v: Value = serde_json::from_str(&body).unwrap();
    let names: Vec<&str> = v["providers"].as_array().unwrap().iter()
        .filter_map(|p| p["name"].as_str())
        .collect();
    assert!(names.contains(&"legit-provider"), "Database should still contain legit-provider after SQL injection attempt");
}
...
async fn xss_name_is_sanitized_in_response() {
    ...
    if status == StatusCode::CREATED {
        ...
        let names: Vec<&str> = v["providers"].as_array().unwrap().iter()
            .filter_map(|p| p["name"].as_str())
            .collect();
        assert!(names.contains(&xss_payload), "XSS payload name should be stored in provider list");
    }
}
...
async fn update_provider_preserves_existing_key() {
    ...
    let req = json_request("PUT", "/api/providers/preserve-key", json!({
        "name": "preserve-key",
        "type": "openai",
        "endpoint": "https://api.openai.com/v1"
    }));
...
async fn update_provider_to_duplicate_name() {
    ...
    let req = json_request("PUT", "/api/providers/orig-b", json!({
        "name": "orig-a",
        "type": "omlx",
        "endpoint": "http://localhost:8000/v1"
    }));
