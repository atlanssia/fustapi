...
pub async fn dispatch_request(protocol: Protocol, body: String, router: &dyn Router) -> Result<Response, ProtocolError> {
    match protocol {
...
async fn openai_handler(body: String, router: &dyn Router) -> Result<Response, ProtocolError> {
    let unified_req = openai::parse_chat_request(&body).map_err(|e| ProtocolError::Parse(e.to_string()))?;
...
async fn forward_streaming(
    ...
    let stream: LLMStream = router.chat_stream(request).await.map_err(|e| ProtocolError::Internal(e.to_string()))?;
...
async fn collect_non_streaming(  
    ...
    let mut stream = router.chat_stream(request).await.map_err(|e| ProtocolError::Internal(e.to_string()))?;  
    ...
    let content = chunks.iter().filter_map(|c| c.content.clone()).collect::<Vec<_>>().join("");  
  
    let response = openai::serialize_response(  
        "chatcmpl-gateway-1", model,   
...
async fn anthropic_handler(_body: String, _router: &dyn Router) -> Result<Response, ProtocolError> {  
    let response = anthropic::serialize_response(  
        "msg-gateway-1", "claude-3", Some("Hello from FustAPI! (mock)"), None, Some("end_turn"),   
    ).map_err(|e| ProtocolError::Internal(e.to_string()))?;  
